...
import { mapValues, assign, isUndefined, clone } from 'lodash';
...
class Schema extends ObjectSchemaType
{
	...
	proxify (obj = {}) {
		...
		const handler = {
			set: (target, key, value) => {
				if (this[_paths][key]) {
					...
					if (!listeners[key])
						this[_paths][key].on('change', (obj, sk, v) => (obj === target[key]) ? this.emit('change', revocable.proxy, key, v, sk) : undefined);
				}
				...
				if (value !== target[key]) {
					...
					if (typeof key !== "symbol")
						this.emit('change', revocable.proxy, key, value);
				}
		...
		this.emit('proxify.before', obj, handler);

		const revocable = Proxy.revocable(obj, handler);

		this.emit('proxify', obj, revocable);
